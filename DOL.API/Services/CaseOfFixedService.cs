...
{
    public class CaseOfFixedService
    {
        private readonly DolContext _context;

        public CaseOfFixedService(DolContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(CaseOfFixedFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.CaseOfFixeds.AsQueryable());
                ...
                List<CaseOfFixed> execute = new List<CaseOfFixed>();
